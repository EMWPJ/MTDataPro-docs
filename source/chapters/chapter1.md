...
| | 退出 | 退出程序 |
| **🔧 工具** | 标定查看 | 查看标定文件内容 |
| | Phoenix TS分割/修复/合并 | Phoenix时间序列处理 |
| | TBL坐标修复/参数替换 | TBL文件处理 |
| | 时间序列转换 | TXT/TSN/DAT/GMT格式互转 |
| | LEMI/Metronix数据处理 | 各仪器数据处理 |
| | EDI处理 | EDI修复和格式转换 |
| | Phoenix格式转换 | CLB/CLC/TBL与JSON互转 |
| | 地震目录/断层编辑 | 地质数据管理 |
| **🧮 正演** | 电偶极子正演 | 电偶极子源正演计算 |
| | MT 2D四叉树正演 | 二维四叉树网格正演 |
| | MT 3D模型 | 三维模型正演 |
| | 合成时间序列 | 生成合成时间序列数据 |
| **⏱️ 时间序列** | SSA/MSSA | 奇异谱分析 |
| | ATTS/MTU降采样 | 数据降采样 |
| **⚙️ 设置** | FFT参数 | 设置FFT处理参数 |
| | 标定路径 | 设置标定文件目录 |
| | 最大线程数 | 设置并行处理线程数 |
| | 语言切换 | 中文/English |
| **❓ 帮助** | 关于 | 查看软件版本和授权信息 |
...
- ℹ️ 菜单 `帮助 → 关于` 查看版本信息
- ❓ 遇到问题请查看 [常见问题FAQ](appendices/appendixD.md)
- 📧 联系技术支持获取帮助
