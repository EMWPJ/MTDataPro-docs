...
1. **多仪器支持**
   ...
   - LEMI长周期仪器
   - 国科 UltraEM Z5/Z5L/Z5-8C系列 ([www.igeoco.com](http://www.igeoco.com))
   - RMT/CASRMT格式
   - Aether (ATTS) 格式
   - EDI格式导入导出
...
MTDP包含12个主菜单，涵盖了数据处理、Forward正演、可视化、标定等全部功能。
...
| | 传感器标定 | 传感器系统响应标定 |
| | 标定查看 | 查看已有标定曲线 |
| **Forward正演** | 1D正演 | 一维地电模型正演计算 |
| | 2D正演 | 二维地电模型正演计算 |
| | 3D正演 | 三维地电模型正演计算 |
| | 读取模型 | 加载已有正演模型 |
| **EDI处理** | 编辑EDI | 编辑EDI文件内容 |
| | 批量处理EDI | 批量处理多个EDI文件 |
| | EDI比较 | 比较两个EDI文件的差异 |
...
MTDP主界面包含5个选项卡容器，共约15个选项卡，用于不同功能模块：
