...
| 💾 | 保存 | 保存当前编辑FC的修改 |
| 💾+ | 保存全部 | 保存所有编辑FC的修改 |
| 📤 | 导出 | 打开导出菜单（EDI/JSON/参数等） |
| 📥 | 导入 | 导入外部数据或参数 |
| 🔄 | 刷新 | 重新加载FC数据，撤销未保存的修改 |
| ⚙️ | 参数设置 | 打开FFT参数或 Robust参数设置 |
| 📊 | 统计信息 | 显示当前FC的统计信息（数据点数、RMS等） |
| 🔍 | 缩放 | 图表缩放控制 |
| 🔲 | 框选模式 | 切换框选/单选模式 |
| ✋ | 剔除模式 | 切换手动剔除模式 |
| ✅ | 恢复模式 | 切换恢复数据模式 |
...
**导出操作：**
- 导出菜单包含多个子选项
- 可以选择导出格式（EDI、JSON、文本等）
- 可以选择导出内容（阻抗、频谱、残差等）

**视图控制：**
- 缩放级别：25%、50%、75%、100%、150%、200%
- 框选模式：用于批量选择数据点进行操作
- 剔除/恢复模式：切换手动标记数据点的操作类型

---

### 左侧面板详解

左侧面板是精细处理界面的"指挥中心"，包含四个主要区域：原始FC列表、编辑FC列表、远参考站列表、反向参考站列表。

#### 区域1：原始FC列表

原始FC列表显示该测点经过FFT处理后生成的所有傅里叶系数文件。
...
| 删除 | 删除FC文件 | 谨慎操作，不可恢复 |
| 合并 | 合并多个FC | 将多个时段合并 |
| 导出JSON | 导出为JSON | 便于程序处理 |
| 导出振幅谱 | 导出振幅数据 | 导出功率谱信息 |
| 属性 | 查看详细信息 | 显示FC的完整参数 |
...
编辑FC列表显示经过精细处理后的各个版本，每个版本记录了不同的处理状态。

**为什么需要多个编辑版本？**

- **保留历史**：每次重要处理前创建新版本，便于回溯
- **对比效果**：同一数据多种处理方法对比
- **协作共享**：不同处理人员可以创建独立版本
...
| 状态 | 含义 | 视觉效果 |
|------|------|----------|
| 原始(Original) | 未经处理的原始FC | 灰色/默认 |
| 已修改(Modified) | 有未保存的修改 | 蓝色高亮 |
| 已保存(Saved) | 修改已保存 | 绿色勾选 |
| 当前(Current) | 当前正在查看/编辑 | 粗体/边框 |
...
| 重命名 | 修改版本名称 | 便于区分不同版本 |
| 删除 | 删除该版本 | 清理不需要的版本 |
| 设为测点FC | 应用到测点 | 处理完成，确认结果 |
| 合并选中 | 合并多个版本 | 整合不同时段 |
| XY合并 | 合并X/Y方向 | 合并正交分量 |
| 复制剔除信息 | 复制剔除标记 | 共享剔除结果 |
| 粘贴剔除信息 | 粘贴剔除标记 | 应用剔除结果 |
| 删除已剔除数据 | 创建干净版本 | 重建无污染数据 |

**版本命名建议：**

```
原始_20240315          # 原始数据，保存日期
马氏距离筛选_v1        # 马氏距离预处理后
RhoPlus_MOGA_30%      # MOGA筛选30%保留
远参考REH_最终        # 远参考处理完成版
对比_筛选前后         # 用于对比的效果图
```
...
|------|------|------|
| 不使用 | None | 默认选项 |
| 本地电场 | LE | 使用本地Ex/Ey作为参考 |
| 本地磁场 | LH | 使用本地Hx/Hy作为参考 |
| 本地电磁 | LEH | 使用本地全电磁场 |
| 远参考电场 | RE | 使用远参考Ex/Ey |
| 远参考磁场 | RH | 使用远参考Hx/Hy |
| 远参考电磁 | REH | 使用远参考全电磁场 |
...
右侧区域包含9个选项卡，用于显示不同类型的处理结果和数据质量信息。

#### 选项卡1：视电阻率/相位（RhoPhs）

**这是最重要的选项卡**，显示MT的核心数据：视电阻率(ρ)和相位(φ)。
...
| 组合 | 说明 | 用途 |
|------|------|------|
| Ex-Hx | 电场Ex与磁场Hx | 主要相干度 |
| Ex-Hy | 电场Ex与磁场Hy | 次要相干度 |
| Ey-Hx | 电场Ey与磁场Hx | 次要相干度 |
| Ey-Hy | 电场Ey与磁场Hy | 主要相干度 |
| Ey-Rx | Ey与远参考Rx | 远参考质量 |
| Ex-Rx | Ex与远参考Rx | 远参考质量 |
...
│  筛选算法选择                                               │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ ○ RhoPlus SA    ○ RhoPlus GA    ● RhoPlus MOGA    │   │
│  │ ○ Full Z MOGA   ○ 马氏距离                           │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  保留比例: [30]%  (滑块: 10% ──●── 90%)                    │
...
| 参数 | 说明 | 推荐值/范围 | 调整建议 |
|------|------|-------------|----------|
| 筛选算法 | 选择使用的算法 | RhoPlus MOGA | 强噪声选MOGA |
| 保留比例 | 最终保留数据的百分比 | 20%-50% | 噪声强则降低 |
...
```
1. 在工具栏点击"剔除模式"按钮（红色✋）
2. 在视电阻率/相位图上，左键框选要剔除的数据点
3. 选中的数据点会变成半透明红色（表示已剔除）
4. 如需恢复，右键点击已剔除区域或切换到"恢复模式"
```
...
| 选项 | 说明 | 输出内容 |
|------|------|----------|
| 阻抗EDI | 标准阻抗格式 | Zxx, Zxy, Zyx, Zyy及误差 |
| 频谱EDI | 含功率谱 | 功率谱、互功率谱 |
| 折叠EDI | 折叠到0-90° | 便于快速查看 |
