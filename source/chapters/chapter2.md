...
> ⚠️ **单位系统说明**：MTDataPro 支持两种单位系统计算视电阻率：
> - **SI单位**（E: V/m, H: A/m）：$\rho_a = |Z|^2 / (\omega \mu_0)$，其中 $\mu_0 = 4\pi \times 10^{-7}$ H/m
> - **CGS单位**（E: mV/km, H: nT）：$\rho_a = 0.2 \times |Z|^2 / f$
> 
> 软件内部自动处理单位转换，输出结果均为标准 SI 单位（Ω·m）。
...
$$Coh_m^2 = \frac{\mathbf{Z}_{:o}^H \mathbf{C}^{-1} \mathbf{Z}_{:o}}{[E_oE_o^*]}$$

其中 $\mathbf{Z}_{:o}$ 是输出与各输入之间的互谱向量，$\mathbf{C}$ 是输入信号的互谱密度矩阵。

*(2.14)*

> 💡 **矩阵形式说明**：重相干度使用矩阵求逆计算，能有效处理多个输入通道间的相关性，比简单相干叠加更准确。
...
> ⚠️ **软件输出说明**：MTDataPro 实际输出的是 $\log_{10}(PSD)$（以 dB 为单位），便于在图上显示宽动态范围的功率谱。判断时注意PSD突然升高一个数量级可能表示噪声干扰。
...
$$(\Delta Z_{ij})^2 = \frac{|\eta_i|^2 \cdot |A_j|^2}{N \cdot |D^R|^2}$$

*(2.17)*

其中：
- $\eta_i$：第 $i$ 个输出通道的预测误差功率（代码中 `etax`、`etay`）
- $A_j$：第 $j$ 个参考通道的功率加权项（代码中 `Ax`、`Ay`）
- $D^R$：远参考站磁场的行列式（代码中 `D2 = |H_x^R H_y^R - H_x^R H_y^R|^2`）
- $N$：叠加次数

> 📝 **代码变量对照表**：
> | 文档符号 | 代码变量 | 含义 |
> |:--------:|:--------:|:-----|
> | ηᵢ | `etax`, `etay` | 输出通道预测误差功率 |
> | Aⱼ | `Ax`, `Ay` | 参考通道功率加权项 |
> | Dᴿ | `D2` | 远参考磁场行列式的模平方 |
> | — | `beta` | 倾子误差功率 |
...
MTDataPro 提供两个辅助方差函数用于误差传播计算：

**VarRho 函数**（视电阻率方差）：

$$\sigma_{\rho}^2 = 0.4 \cdot \frac{\rho \cdot \sigma_Z^2}{f}$$

其中 $\rho$ 为视电阻率，$\sigma_Z^2$ 为阻抗方差，$f$ 为频率。

**VarPhs 函数**（相位方差）：

$$\sigma_{\varphi}^2 = 8 \cdot \cos^4(\varphi) \cdot \frac{|Z|^2 \cdot \sigma_Z^2}{(2Z_R)^4}$$

其中 $\varphi$ 为相位，$Z_R$ 为阻抗实部。

> 📝 **代码实现**：这两个函数定义于 `MTFunctions.pas` 第 304-312 行，分别用于从阻抗方差计算视电阻率和相位的误差传播。
...
> 📝 **代码实现说明**：软件中相位张量元素记为 `Pxx, Pxy, Pyx, Pyy`（对应数学符号 Φₓₓ, Φₓᵧ, Φᵧₓ, Φᵧᵧ`），使用 `arctan2` 函数计算 α 和 β，结果以**度**为单位输出。
...
| **重相干度** | $Coh_m^2 = \mathbf{Z}_{:o}^H \mathbf{C}^{-1} \mathbf{Z}_{:o} / [E_oE_o^*]$ | (2.14) |
...
| **主方向角** | $\alpha = \frac{1}{2}\arctan2(\Phi_{xy}+\Phi_{yx}, \Phi_{xx}-\Phi_{yy}) \times \frac{180°}{\pi}$ | (2.21) |
| **二维偏离度** | $\beta = \frac{1}{2}\arctan2(\Phi_{xy}-\Phi_{yx}, \Phi_{xx}+\Phi_{yy}) \times \frac{180°}{\pi}$ | (2.22) |
...
| **Gamble方差** | $(\Delta Z_{ij})^2 = \frac{|\eta_i|^2 \cdot |A_j|^2}{N \cdot |D^R|^2}$ | (2.17) |
---
